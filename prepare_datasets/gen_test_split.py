...
if __name__ == "__main__":
    np.random.seed(99)  # 使用不同的隨機種子
    
    # ========== 配置參數 ==========
    num_test = 640  # 測試集數量（boy3 + girl3）
    nfill = len(str(num_test))
    ...
    save_root = '../datasets/test_data/'
    
    # 輸入 CSV 文件（測試集專用）
    data_root = './'
    clean_csv = os.path.join(data_root, 'test_clean_dir.csv')
    noise_csv = os.path.join(data_root, 'test_noise_dir.csv')
    
    # ========== 讀取文件列表 ==========
    print("=" * 60)
    print("Loading file lists for TEST set...")
    print("=" * 60)
    
    clean_list = pd.read_csv(clean_csv)['file_dir'].tolist()
    noise_list = pd.read_csv(noise_csv)['file_dir'].tolist()
    snr_list = np.random.uniform(snr_range[0], snr_range[1], size=num_test)
    
    print(f"Clean files: {len(clean_list)}")
    print(f"Noise files: {len(noise_list)}")
    print(f"Target length: {wav_len} seconds ({target_length} samples)")
    print(f"Speakers: boy3, girl3 ONLY")
    print(f"Noise: factory2.wav ONLY")

    
    # ========== 創建輸出目錄 ==========
    os.makedirs(os.path.join(save_root, 'test_noisy'), exist_ok=True)
    os.makedirs(os.path.join(save_root, 'test_clean'), exist_ok=True)
    
    # ========== 保存配置信息 ==========
    info = pd.DataFrame([str(idx+1).zfill(nfill)+'.wav' for idx in range(num_test)], 
                        columns=['file_name'])
    info['clean'] = clean_list
    info['noise'] = noise_list
    info['snr'] = snr_list
    
    info.to_csv(os.path.join(save_root, 'test_INFO.csv'), index=None)
    
    # ========== 生成測試數據 ==========
    print("\n" + "=" * 60)
    print(f"Generating {num_test} test samples (without reverberation)...")
    print("=" * 60)
    ...
    for idx in tqdm(range(num_test)):
        try:
            ...
            # 儲存
            output_noisy = os.path.join(save_root, 'test_noisy', 
                                       str(idx+1).zfill(nfill)+'.wav')
            output_clean = os.path.join(save_root, 'test_clean', 
                                       str(idx+1).zfill(nfill)+'.wav')
    ...
    print(f"Noisy files: {save_root}test_noisy/")
    print(f"Clean files: {save_root}test_clean/")
    print(f"Info file: {save_root}test_INFO.csv")
    print("\n⚠️  IMPORTANT: Test set uses UNSEEN speakers (boy3, girl3) and noise (factory2)")
