...
if __name__ == "__main__":
    ...
    # ========== 配置參數 ==========
    total_files = 1280  # 訓練+驗證總數（排除測試集）
    ...
    # ========== 讀取文件列表 ==========
    print("=" * 60)
    print("Loading file lists for TRAINING set (90% of train/val)...")
    print("=" * 60)
    
    # 只取前 90% 作為訓練集
    clean_list = pd.read_csv(clean_csv)['file_dir'].tolist()[:num_train]
    noise_list = pd.read_csv(noise_csv)['file_dir'].tolist()[:num_train]
    snr_list = np.random.uniform(snr_range[0], snr_range[1], size=num_train)
    
    print(f"Clean files: {len(clean_list)} (indices 0-{num_train-1})")
    ...
    # ========== 保存配置信息 ==========
    info = pd.DataFrame([str(idx+1).zfill(nfill)+'.wav' for idx in range(num_train)], 
                        columns=['file_name'])
    ...
    # ========== 生成訓練數據 ==========
    print("\n" + "=" * 60)
    print(f"Generating {num_train} training samples (without reverberation)...")
    print("=" * 60)
    
    for idx in tqdm(range(num_train)):
        try:
            # 讀取文件
