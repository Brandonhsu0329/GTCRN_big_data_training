...
def process_csv_and_generate_audio(csv_file, output_dir, data_type):
    ...
    for row in tqdm(rows, desc=f"生成 {data_type} 音訊"):
        ...
        try:
            ...
            # 確保採樣率一致（應該都是 16kHz）
            if sr_clean != 16000:
                print(f"\n警告: {clean_path} 採樣率為 {sr_clean}, 預期為 16000")
            if sr_noise != 16000:
                print(f"\n警告: {noise_path} 採樣率為 {sr_noise}, 預期為 16000")
...
def main():
    base_path = "/home/sbplab/yuchen/GTCRN/SEtrain/datasets3"
    
    # 設定隨機種子以確保可重現性
    np.random.seed(42)
    
    print("="*60)
    print("開始生成 datasets3 混合音訊資料集")
    print("使用 speech_lib_bigger 資料")
    print("="*60)
    ...
    print("│   ├── train_noisy/  (192,000 個含噪音的音訊)")
    print("│   └── train_clean/  (192,000 個乾淨的音訊)")
    print("├── val_data/")
    print("│   ├── val_INFO.csv")
    print("│   ├── val_noisy/  (1,600 個含噪音的音訊)")
    print("│   └── val_clean/  (1,600 個乾淨的音訊)")
    print("└── test_data/")
    print("    ├── test_INFO.csv")
    print("    ├── test_noisy/  (6,400 個含噪音的音訊)")
    print("    └── test_clean/  (6,400 個乾淨的音訊)")
    print(f"\n總計: 200,000 個混合音訊檔案")
